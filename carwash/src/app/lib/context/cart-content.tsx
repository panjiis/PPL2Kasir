...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const [billOption, setBillOption] = React.useState<"Dine In" | "Take Away" | null>(null)
  const [locked, setLocked] = React.useState(false)
  const [paymentSheetOpen, setPaymentSheetOpen] = React.useState(false)

  const products = React.useMemo(() => items.filter((i) => i.type === "product"), [items])
  const services = React.useMemo(() => items.filter((i) => i.type === "service"), [items])

  const subtotal = React.useMemo(() => items.reduce((sum, i) => sum + i.price * i.qty, 0), [items])
  const tax = React.useMemo(() => subtotal * 0.1, [subtotal])
  const discount = 0
  const total = React.useMemo(() => subtotal + tax - discount, [subtotal, tax, discount])
  ...
  const setEmployee = React.useCallback((id: string, name: string) => {
    setItems((prev) => prev.map((i) => (i.id === id ? { ...i, employee: name } : i)))
  }, [])
