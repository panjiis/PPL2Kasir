...
interface PreferencesContextType {
  ...
  getButtonColorClasses: (key: string) => {
    bg: string;
    text: string;
    border?: string;
  };
...
const COLOR_OPTIONS = [
  ...
  },
  {
    key: 'accent',
    label: 'Accent',
    classes: { bg: 'bg-accent', text: 'text-accent-foreground' },
  },
  {
    key: 'muted',
    label: 'Muted',
    classes: { bg: 'bg-muted', text: 'text-muted-foreground' },
  },
];
...
export function PreferencesProvider({
  ...
  const [buttonPrefs, setButtonPrefs] = React.useState<
    Record<string, ButtonPref>
  >({});
